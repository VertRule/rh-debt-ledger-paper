...
echo "[2/25] Checking for Co-Authored-By trailers..."
...
echo "[4/25] Fetching and comparing with origin/main..."
...
echo "[5/25] Checking required files..."
...
echo "[6/25] Checking forbidden artifacts are not tracked..."
...
echo "[7/25] Running redaction scan..."
...
echo "[8/25] Running exhibit digest verification..."
...
echo "[9/25] Running verify-all-exhibits..."
...
echo "[10/25] Verifying proof artifacts..."
...
echo "[11/25] Checking contribution ledger..."
...
echo "[12/25] Verifying R4 transfer packet..."
...
echo "[13/25] Verifying R5 error-bound source..."
...
echo "[14/25] Verifying R6 instantiation record..."
...
echo "[15/25] Verifying R7 bound statement..."
...
echo "[16/25] Verifying R8 comparison run..."
...
echo "[17/25] Verifying R9 no-surprise assembly..."
...
echo "[18/25] Verifying R10 assembly receipt..."
...
echo "[19/25] Verifying R11 optional signature..."
...
echo "[20/25] Verifying R14 key capture receipts..."
...
echo "[21/25] Verifying R15 optional PQ signatures..."
...
echo "[22/25] Verifying R16 PQ tooling receipt..."
...
# 23) Verify R20 assembly root rebuild (soft dependency)
echo "[23/25] Verifying R20 assembly root rebuild..."
if [ -x "./VERIFY_R20_REBUILD_ASSEMBLY_ROOT.sh" ]; then
    set +e
    R20_OUTPUT=$(./VERIFY_R20_REBUILD_ASSEMBLY_ROOT.sh 2>&1)
    R20_EXIT=$?
    set -e
    if [ "$R20_EXIT" -eq 0 ]; then
        echo "  OK: R20 assembly root rebuild verified"
    else
        echo "$R20_OUTPUT"
        error "R20 assembly root rebuild verification failed"
    fi
else
    echo "  SKIP: VERIFY_R20_REBUILD_ASSEMBLY_ROOT.sh not found or not executable"
fi

# 24) Verify R25 paper PDF digest
echo "[24/25] Verifying R25 paper PDF digest..."
if [ -x "./VERIFY_R25_PAPER_PDF_DIGEST.sh" ]; then
    set +e
    R25_OUTPUT=$(./VERIFY_R25_PAPER_PDF_DIGEST.sh 2>&1)
    R25_EXIT=$?
    set -e
    if [ "$R25_EXIT" -eq 0 ]; then
        echo "  OK: R25 paper PDF digest verified"
    else
        echo "$R25_OUTPUT"
        error "R25 paper PDF digest verification failed"
    fi
else
    echo "  SKIP: VERIFY_R25_PAPER_PDF_DIGEST.sh not found or not executable"
fi

# 25) Verify R31 no token leak
echo "[25/25] Verifying R31 no token leak..."
if [ -x "./VERIFY_R31_NO_TOKEN_LEAK.sh" ]; then
    set +e
    R31_OUTPUT=$(./VERIFY_R31_NO_TOKEN_LEAK.sh 2>&1)
    R31_EXIT=$?
    set -e
    if [ "$R31_EXIT" -eq 0 ]; then
        echo "  OK: R31 no token leak verified"
    else
        echo "$R31_OUTPUT"
        error "R31 no token leak verification failed"
    fi
else
    echo "  SKIP: VERIFY_R31_NO_TOKEN_LEAK.sh not found or not executable"
fi
